...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  const p = projRes.data as unknown as ProjectWithJoins | null;
  if (!p) notFound();

  const learner  = p.learners;
  const profile  = learner?.learner_profiles;
  const stage    = STAGES.find(s => s.key === (p.stage || 'planning')) || STAGES[0];
  const pct      = p.score != null ? Math.round((p.score / (p.max_score || 100)) * 100) : null;
  const feedback = ((feedbackRes.data || []) as unknown as FeedbackRow[])
    .filter(f => !f.is_private || user.role === 'admin' || user.role === 'instructor')
    .map(f => ({ ...f, users: Array.isArray(f.users) ? f.users[0] ?? {} : f.users ?? {} }));
  const overdue  = p.due_date && new Date(p.due_date) < new Date() && !['submitted','marked'].includes(p.stage ?? '');
    ...
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: DS.text }}>{p.project_name}</h1>
            <p className="text-sm mt-0.5" style={{ color: DS.textMuted }}>
              {profile?.first_name} {profile?.last_name} · {p.programs?.program_name}
            </p>
          </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-2xl p-5 space-y-4" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
            {p.description && (
              <div>
                ...
                <p className="text-sm" style={{ color: DS.textMid }}>{p.description}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium" style={{ color: DS.text }}>{p.programs?.program_name}</p>
                <p className="text-xs" style={{ color: DS.textMuted }}>{p.programs?.program_type}</p>
              </div>
              {p.due_date && (
                <div>
                  ...
                  <p className="font-medium flex items-center gap-1"
                    ...
                    <Calendar className="w-3.5 h-3.5" />{fmt.date(p.due_date)}
              ...
              {p.submitted_at && (
                <div>
                  ...
                  <p className="font-medium" style={{ color: DS.text }}>{fmt.date(p.submitted_at)}</p>
                </div>
              )}
            </div>
          ...
          <div className="rounded-2xl p-5" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
            ...
            <div className="flex items-center gap-1 flex-wrap">
              {STAGES.map((s, i) => {
                const isActive  = s.key === p.stage;
                const isPassed  = STAGES.findIndex(st => st.key === p.stage) > i;
                return (
