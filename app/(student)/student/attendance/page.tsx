...
export default async function StudentAttendancePage() {
  ...
  const { data: learner } = await supabase
    .from('learners')
    .select(`attendance(attendance_id, status, session_date, notes, programs(program_name))`)
  ...
  const STATUS: Record<string, { color: string; bg: string; emoji: string; label: string }> = {
    present: { color: '#2DD4A0', bg: 'rgba(45,212,160,0.12)',  emoji: '✅', label: 'Present' },
    absent:  { color: '#F87171', bg: 'rgba(248,113,113,0.12)', emoji: '❌', label: 'Absent' },
    late:    { color: '#FCD34D', bg: 'rgba(252,211,77,0.12)',  emoji: '⏰', label: 'Late' },
    excused: { color: '#60A5FA', bg: 'rgba(96,165,250,0.12)',  emoji: '📋', label: 'Excused' },
  };
  ...
  return (
    <div className="space-y-5 pt-2">
      <div>
        <h1 className="text-2xl font-black" style={{ color: t.text }}>My Attendance 📅</h1>
        <p className="text-sm mt-0.5" style={{ color: t.muted }}>{total} sessions recorded</p>
      </div>

      {/* Attendance ring */}
      <div className="rounded-3xl p-6 relative overflow-hidden"
        style={{ background: `${attColor}12`, border: `1px solid ${attColor}35` }}>
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1">
            <p className="text-2xl font-black" style={{ color: t.text }}>{rate}%</p>
            <p className="text-sm mt-0.5" style={{ color: t.muted }}>Attendance Rate</p>
      ...
      {records.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="font-bold" style={{ color: t.text }}>No sessions recorded yet</p>
          <p className="text-sm mt-1" style={{ color: t.muted }}>Your attendance will appear here</p>
        </div>
      ) : (
        <div>
