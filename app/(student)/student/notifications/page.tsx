  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: t.muted }}>{notifications.length} recent updates</p>
      </div>

      {notifications.length === 0 ? (
        <div className="text-center py-16">
          <Bell className="w-12 h-12 mx-auto mb-3" style={{ color: t.muted }} />
          <p style={{ color: t.muted }}>No notifications yet.</p>
          <p className="text-sm mt-1" style={{ color: t.muted }}>
            Updates will appear here when your teacher records marks or sessions.
          </p>
        </div>
      ) : (
          ...
          {notifications.map((n, i) => (
            <div key={i} className="rounded-2xl p-4 flex items-start gap-3"
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: t.text }}>{n.title}</p>
                <p className="text-xs mt-0.5" style={{ color: t.muted }}>{n.body}</p>
                {n.time && (
                  <p className="text-xs mt-1" style={{ color: t.muted }}>{fmt.date(n.time)}</p>
                )}
              </div>
