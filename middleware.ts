...
export async function middleware(request: NextRequest) {
  ...
  // Portal-to-role enforcement: redirect users who hit the wrong portal.
  ...
  if (!profile) return NextResponse.next({ request: { headers: request.headers } });

  if (!profile.is_active && !pathname.startsWith('/login')) {
  ...
  // Block a learner from reaching /dashboard or /teacher, etc.
  const wrongPortal =
